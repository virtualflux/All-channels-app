...
const CustomersPage = () => {
    ...
    const fetchCustomers = async (currentPage: number) => {
      try {
        const res = await axios.get<{
          ...
          data: ICustomer[];
          count: number;
        }>(`/api/db/customers?page=${currentPage}`, {});
    ...
    const { data, isLoading, error, refetch } = useQuery({
     ...
     const approveCustomer = async (customerId: string) => {
       setBusyAction({ id: customerId, action: "approve" });
       const body: UpdateStatusPayload = { status: "approved" };
       await axios
         .put(`/api/db/customers/${customerId}`, body)
         .then((res) => {
     ...
     const rejectCustomer = async (customerId: string) => {
       setBusyAction({ id: customerId, action: "reject" });
       const body: UpdateStatusPayload = { status: "rejected" };
       await axios
         .put(`/api/db/customers/${customerId}`, body)
         .then((res) => {
           toast.success("Rejected account");
           refetch();
         })
         .catch((error) => {})
         .finally(() => {
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              Approve or reject customers to create them in Zoho Books
            </p>
          </div>
                  ...
                  <div className="font-medium">
                    <ul className="mb-2 last:mb-0">
                      <li className="text-sm">
                        <div className="font-medium">
                          {record.contact_persons[0].first_name}{" "}
                          {record.contact_persons[0].last_name}
                        </div>
                        <div className="text-gray-500">
                          {record.contact_persons[0].email}
                        </div>
                        <div className="text-gray-500">
                          {record.contact_persons[0].phone}
                        </div>
                      </li>
                    </ul>
                  </div>
