...
const ProductsPage = () => {
  ...
  const updateStatus = async (id: string, status: IProduct["status"]) => {
    setBusyAction({ id, action: status === "approved" ? "approve" : "reject" });
          ...
          return (
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${
                v ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-600"
              }`}
            >
              {v ? "Yes" : "No"}
            </span>
      ...
      },
      {
        ...
        cell: (info: any) => {
          ...
          const loadingApprove =
            busyAction?.id === product._id && busyAction?.action === "approve";
          const loadingReject =
            busyAction?.id === product._id && busyAction?.action === "reject";
...
function DetailsModal({ open, record, onClose }: DetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60]">
      ...
      >
        <div className="w-full h-3/4 overflow-y-auto max-w-3xl rounded-xl bg-white shadow-xl ">
          <div className="  ">
                ...
                record.locations.map((location) => (
                  <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-zinc-600">
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {location.location_stock_on_hand ?? 0}
                      </span>
                    </div>
                    <div>
