...
const AppTable = <TData,>({
  ...
  return (
    <div className="overflow-x-auto p-1 text ">
      <DebouncedInput value={""} onChange={(val) => table.setGlobalFilter(val)} type="text"
        placeholder="Search..."
          ...
          {table.getRowModel().rows.length ? table.getRowModel().rows.map(row => (
            ...
            <td colSpan={100} className="py-12">
              <div className="w-full flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-gray-400 mt-1">There are no records to display</p>
              </div>
            </td>
          </tr>)}
        </tbody>
      ...
      <div className="mt-4 flex justify-between items-center">
        <p className='font-semibold'>Count: <span className='ml-1'> {table.getRowCount()}</span></p>
      </div>
