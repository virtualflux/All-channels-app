...
const SearchableDropdown = ({
    ...
    value: string
    options: { name: string; value: any }[]; isLoading?: boolean; onSelect: (value: { name: string; value: any }) => void;
    placeholder?: string;
    loadingText?: string;
    ...
    const handleSelect = (option: { name: string; value: any }) => {
        const pickedOption = options.find(item => item.value == option.value)
        if (pickedOption) {
   ...
   useEffect(() => {
    ...
    return (
        <div className="relative w-full" ref={dropdownRef}>
            <div
                className={`flex items-center justify-between p-3 border border-gray-300 rounded-md ${disabled
            ...
            {isOpen && !disabled && (
                <div className="absolute z-50 w-full mt-1 bg-primary border border-gray-300 rounded-md shadow-lg bg-white">
